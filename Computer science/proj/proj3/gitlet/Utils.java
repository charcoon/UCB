...
class Utils {
    ...
    private static String sha1(Object... vals) {
        try {
            ...
            for (Object val : vals) {
                if (val instanceof byte[]) {
                    ...
                    md.update(val.toString().getBytes(StandardCharsets.UTF_8));
                } else if (val instanceof Commit) {
                    md.update((((Commit) val).hash())
     ...
     * @return the hash.
     */
    static <T> String sha1(Collection<T> vals) {
     ...
     * @return the hash.
     */
    static <T> String sha1(String a, String b) {
        return sha1(List.of(a, b));
    }
     ...
     * VALS.
     * @param vals the vals.
     * @param vals2 the vals2.
     * @param <T1> the type.
     * @param <T2> the type.
     * @return the hash.
     */
    static <T1, T2> String sha1(Collection<T1> vals, Collection<T2> vals2) {
        return sha1(sha1(vals.toArray()), sha1(vals2.toArray()));
    }
