    ...
    static PlaceList[][][] successorCells(int width, int height) {
        ...
        for (int i = 0; i < width; i++) {
            for (int j = 0; j < height; j++) {
                List<Place> allSuccessors = new ArrayList<>();
                for (int k = 1; k <= 8; k++) {
                    int x = i, y = j;
                    List<Place> successors = new ArrayList<>();
                    while (true) {
                        x += dx(k);
                        y += dy(k);
                        ...
                        successors.add(new Place(x, y));
                        allSuccessors.add(new Place(x, y));
                    }
                    M[i][j][k] = new PlaceList(successors);
                }
                M[i][j][0] = new PlaceList(allSuccessors);
            }
        }
        return M;
    }
