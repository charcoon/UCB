    ...
    def valid(self):
        ...
        for tower in self.towers:
            if not 0 <= tower.x < self.instance.grid_side_length:
                print(f"Tower out of bounds: {str(tower)}, {self.instance.grid_side_length}")
                return False
        ...
        for city in self.instance.cities:
            for tower in self.towers:
                ...
                print("City not covered:", city)
                return False

        if not len(set(self.towers)) == len(self.towers):
            print("Duplicate towers:", self.towers)
            return False
