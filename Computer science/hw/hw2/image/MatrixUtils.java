...
public class MatrixUtils {
    ...
    public static double[][] accumulateVertical(double[][] m) {
        double[][] ANS = MatrixUtils.copy(m);
        if (m == null || m.length <= 1) { return m; }

        for (int i = 1; i < m.length; i ++) {
            for (int j = 0; j < m[0].length; j ++) {
                double M1 = Double.min(get(ANS, i - 1, j - 1), get(ANS, i - 1, j));
                double M2 = Double.min(M1, get(ANS, i - 1, j + 1));
                ANS[i][j] += M2;
            }
        }
    ...
    private static double get(double[][] A, int B, int C) {
        if (B >= 0 && B < A.length && C >=0 && C < A[0].length) {
            return A[B][C];
        } else {
    ...
    public static double[][] accumulate(double[][] m, Orientation orientation) {
        ...
        return transpose(accumulateVertical(transpose(m))); //your code here
    }
